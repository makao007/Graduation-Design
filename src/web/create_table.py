...
cur.execute ("create table if not exists weburls  (id serial primary key, url varchar(1024) not null, title varchar(1000), last_modify char(30), download_time timestamp default now(), content text, description varchar(10240), splited boolean default false); ")
...
cur.execute ("create table if not exists weburl_content_split (id serial primay key, url_id serial references weburls (id) title varchar(2048), content text, description varchar(10240)); ")
